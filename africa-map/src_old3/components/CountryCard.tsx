...
export default function CountryCard({ result, showCustom, isSelected, onToggleCompare, canAddCompare }: Props) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{
          ...
          background: result.excluded ? '#2a1a1a' : '#1e2535',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '0.9rem', fontWeight: 700,
          color: result.excluded ? '#7a3a3a' : '#7db4f0',
        }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            {result.excluded && (
              <span style={{ fontSize: '0.72rem', padding: '0.1rem 0.4rem', borderRadius: 4, background: '#3a1a1a', color: '#d0604a' }}>Excluded</span>
            )}
          </div>
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', color: '#4a5568', cursor: 'pointer', fontSize: '1rem', padding: '0.25rem', lineHeight: 1 }}
        >
          {expanded ? '▲' : '▼'}
        </button>
      </div>
          ...
          {PILLARS.map(pd => {
            ...
            return (
              <div key={pd.id} title={`${pd.shortName}: ${p.score}/100`} style={{
                flex: 1, height: `${Math.max(4, Math.round(p.score * 0.28))}px`,
                background: pd.color, borderRadius: 2, opacity: 0.8, alignSelf: 'flex-end',
              }} />
            );
          })}
        </div>
          ...
          {result.excluded && result.exclusionReasons.length > 0 && (
            <div style={{ marginBottom: '0.75rem' }}>
              ...
              {result.exclusionReasons.map((r, i) => (
                <div key={i} style={{ fontSize: '0.8rem', color: '#a0504a', marginBottom: '0.15rem' }}>• {r}</div>
              ))}
            </div>
          )}
                ...
                <div key={pd.id} style={{ marginBottom: '0.8rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.2rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {evidence && (
                        <button
                          onClick={() => setActiveEvidencePillar(isEvidenceOpen ? null : pd.id)}
                          style={{
                            ...
                            cursor: 'pointer', fontSize: '0.65rem', padding: '0.1rem 0.4rem',
                            fontFamily: 'inherit',
                          }}
                        >📚 Research</button>
                      )}
                  ...
                  {!isPending && p.drivers.map((d, i) => (
                    <div key={i} style={{ fontSize: '0.72rem', color: '#4a5568', marginTop: '0.1rem' }}>· {d}</div>
                  ))}
                  {isEvidenceOpen && evidence && (
                    <div style={{ marginTop: '0.5rem', background: '#0d1117', border: `1px solid ${pd.color}30`, borderRadius: 6, padding: '0.75rem' }}>
                      <div style={{ fontSize: '0.75rem', color: pd.color, fontWeight: 600, marginBottom: '0.4rem' }}>
                        Research basis — {evidence.confidence === 'strong' ? '✓ Strong evidence' : evidence.confidence === 'moderate' ? '~ Moderate evidence' : '⚡ Mixed/pending'}
                      </div>
                      <div style={{ fontSize: '0.78rem', color: '#a0aec0', lineHeight: 1.6, marginBottom: '0.6rem' }}>
                        {evidence.keyInsight}
                      </div>
                      ...
                      {evidence.citations.map((c, i) => (
                        <div key={i} style={{ background: '#161c27', borderRadius: 4, padding: '0.4rem 0.6rem', marginBottom: '0.3rem' }}>
                          <div style={{ fontSize: '0.72rem', fontWeight: 600, color: '#7db4f0' }}>
                            {c.authors} ({c.year}) — {c.journal}
                          </div>
                          <div style={{ fontSize: '0.72rem', color: '#a0aec0', marginTop: '0.15rem' }}>{c.finding}</div>
                        </div>
                      ))}
                      <div style={{ fontSize: '0.7rem', color: '#4a5568', marginTop: '0.4rem', fontStyle: 'italic' }}>
                        {evidence.yourDataNote}
                      </div>
                    </div>
                  )}
                </div>
