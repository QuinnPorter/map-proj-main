...
export const PILLARS: { id: keyof PillarWeights; name: string; shortName: string; color: string }[] = [
  ...
  { id: 'businessEnv',  name: 'Business Env. & Firm Growth',      shortName: 'Business Env.', color: '#7a6b9e' },
];

// ─── Default weights ──────────────────────────────────────────────────────────

export const DEFAULT_WEIGHTS: PillarWeights = {
  political: 14, ruleOfLaw: 16, fx: 13, macro: 12,
  marketDepth: 13, infrastructure: 11, growth: 12, businessEnv: 9,
};
...
function buildPillars(d: (typeof COUNTRY_DATA)[0]): PillarScore[] {
  ...
  const growth: PillarScore = {
    ...
    drivers: [
      ...
      `Natural resources: ${d.naturalResources ? d.naturalResources.substring(0, 60) : 'N/A'}`,
    ],
  };

  const businessEnv: PillarScore = {
    id: 'businessEnv', name: 'Business Env. & Firm Growth',
    score: d.pillar_businessEnv, weight: 0, confidence: 'medium',
    drivers: [
      `Sanctions: ${d.sanctionsLevel || 'Clean'} — ${d.sanctionsNotes ? d.sanctionsNotes.substring(0, 70) : 'See detail'}`,
      `Nationalisation risk: ${d.nationalisationRisk || 'Medium'}`,
      `Competition law quality: ${d.competitionLawScore || 3}/10`,
    ],
  };

  return [political, ruleOfLaw, fx, macro, marketDepth, infrastructure, growth, businessEnv];
}
...
function checkFilters(d: (typeof COUNTRY_DATA)[0], filters: HardFilters): string[] {
  ...
  // Sanctions check
  if (filters.excludeSanctionedCountries) {
    const sl = (d.sanctionsLevel || '').toLowerCase();
    if (sl.includes('high risk') || sl.includes('very high risk')) {
      reasons.push(`Active sanctions: ${d.sanctionsLevel} (your red line)`);
    }
  }

  // Nationalisation check
  if (filters.excludeHighNationalisationRisk) {
    const nr = d.nationalisationRisk || '';
    if (nr === 'Very High' || nr === 'High') {
      reasons.push(`Nationalisation risk: ${nr} (your red line)`);
    }
  }
...
export function scoreCountries(
  ...
  filters: HardFilters = {
    ...
    excludeSanctionedCountries: false,
    excludeHighNationalisationRisk: false,
  },
): CountryResult[] {
  // Build results
  const results: CountryResult[] = COUNTRY_DATA.map(d => {
    ...
    return {
      ...
      sanctionsLevel: d.sanctionsLevel, sanctionsUN: d.sanctionsUN,
      sanctionsUS: d.sanctionsUS, sanctionsEU: d.sanctionsEU,
      sanctionsNotes: d.sanctionsNotes,
      nationalisationRisk: d.nationalisationRisk, nationalisationNotes: d.nationalisationNotes,
      competitionLawScore: d.competitionLawScore,
