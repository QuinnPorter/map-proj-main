...
export default function CountryCard({ result, showCustom }: Props) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${result.excluded ? '#4a1c1c' : '#2a3245'}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ fontWeight: 600, color: '#e8ecf0', fontSize: '1rem' }}>
              {result.name}
            </span>
            ...
            {result.excluded && (
              <span style={{
                fontSize: '0.72rem', padding: '0.1rem 0.4rem',
                borderRadius: 4, background: '#3a1a1a', color: '#d0604a',
              }}>Excluded</span>
            )}
          </div>
        ...
        {!result.excluded && (
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '1.4rem', fontWeight: 700, color: '#e8ecf0', lineHeight: 1 }}>
              {score}
            </div>
        ...
        <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.3rem', alignItems: 'flex-end', height: 28 }}>
          {PILLARS.map(pd => {
            ...
            if (!p || p.score < 0) return (
              <div key={pd.id} style={{
                flex: 1, height: 4, background: '#1e2535',
            ...
            return (
              <div key={pd.id} title={`${pd.shortName}: ${p.score}/100`} style={{
                ...
                background: pd.color, borderRadius: 2,
                opacity: 0.8, alignSelf: 'flex-end',
              }} />
            );
          })}
        </div>
        ...
        <div style={{ marginTop: '1rem', borderTop: '1px solid #2a3245', paddingTop: '1rem' }}>
          ...
          {result.excluded && result.exclusionReasons.length > 0 && (
            <div style={{ marginBottom: '0.75rem' }}>
              <div style={{ fontSize: '0.75rem', color: '#d0604a', fontWeight: 600, marginBottom: '0.3rem' }}>
                Excluded because:
              </div>
          ...
          <div style={{ marginBottom: '0.75rem' }}>
            ...
            {PILLARS.map(pd => {
              ...
              return (
                <div key={pd.id} style={{ marginBottom: '0.6rem' }}>
                  ...
                  {!isPending && (
                    <div style={{ height: 4, background: '#1e2535', borderRadius: 2 }}>
                      <div style={{
                        ...
                        background: pd.color, borderRadius: 2, opacity: 0.85,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
            {[
              ...
              <div key={label} style={{
                background: '#1e2535', borderRadius: 6, padding: '0.5rem 0.75rem',
              }}>
